...
### 1.3 `src/pages/WorkflowList.tsx` — 3 dead navigation buttons
...
### 3.6 `src/pages/Settings.tsx` — Profile form input values not italic
- **Issue:** `FormInput` receives `className="italic"` but this applies to the wrapper `<div>`, not the `<input>` element's text.
- **Where:** `src/components/ui-custom/FormInput.tsx` line 22 — the `className` prop goes to the outer div.
- **Fix:** When the input has a `defaultValue` or `value` that is mock data, apply `italic` to the `<input>` element itself via the className pass-through.

### 3.7 `src/pages/ShowcasePage.tsx` — StatCard values not italic
- **Issue:** `ui-custom/StatCard.tsx` renders `value` prop without `italic` class.
- **Where:** `src/components/ui-custom/StatCard.tsx` line 34 — `<p className="text-2xl font-bold text-white">{value}</p>`
- **Fix:** Add `italic` class to the value `<p>` tag.

### 3.8 `src/pages/ShowcasePage.tsx` — ActivityItem mock data not italic
- **Issue:** ActivityItem titles, descriptions, and timestamps rendered as normal text.
- **Where:** `src/components/ui-custom/ActivityItem.tsx` — text rendering elements.
- **Fix:** Add `italic` to mock data text spans.

### 3.9 `src/pages/Dashboard.tsx` — ExecutionChart SVG labels not italic
- **Issue:** Chart axis labels ("Jan", "Feb", etc.) are mock data but rendered in normal style.
- **Where:** `src/components/dashboard/ExecutionChart.tsx` line 69–72 — `<span>` elements for labels.
- **Fix:** Add `italic` class to label spans.
...
### 4.5 `src/components/workflow/SearchBar.tsx` — Clear button verification
- **Fix:** Verify X button appears with text and clears input. (Likely already working — confirm.)

### 4.6 `src/components/workflow/WorkflowCard.tsx` — Italic applied to UI labels
...
**Total: 18 files, 41 fixes, 4 phases**
