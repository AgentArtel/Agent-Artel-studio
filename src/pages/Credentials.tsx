  ...
  return (
    <div className="min-h-screen bg-dark text-white p-6">
      <div className="flex items-center justify-between mb-6">
        ...
        <Button className="bg-green text-dark hover:bg-green-light" onClick={() => setIsAddModalOpen(true)}>
      ...
      {filteredCredentials.length > 0 ? (
        ...
        <EmptyState title="No credentials found" description="Add your first API key to connect with external services" icon={<Key className="w-8 h-8" />} actionLabel="Add Credential" onAction={() => setIsAddModalOpen(true)} />
      )}

      <Modal isOpen={isAddModalOpen} onClose={() => setIsAddModalOpen(false)} title="Add Credential" description="Add a new API key or service connection">
        <div className="space-y-4">
          <div>
            <label className="text-xs text-white/50 uppercase tracking-wider mb-2 block">Name</label>
            <input type="text" placeholder="e.g., OpenAI Production" className="w-full px-4 py-3 bg-dark-200 border border-white/10 rounded-lg text-sm text-white placeholder:text-white/30 focus:outline-none focus:border-green/50" />
          </div>
          <div>
          ...
          <div>
            ...
            <input type="password" placeholder="Enter your API key" className="w-full px-4 py-3 bg-dark-200 border border-white/10 rounded-lg text-sm text-white placeholder:text-white/30 focus:outline-none focus:border-green/50" />
          </div>
          <div className="flex justify-end gap-3 pt-4">
            <Button variant="ghost" onClick={() => setIsAddModalOpen(false)}>Cancel</Button>
            <Button className="bg-green text-dark hover:bg-green-light">Add Credential</Button>
          </div>
        </div>
      </Modal>
    </div>
