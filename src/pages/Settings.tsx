        ...
        <div className="lg:col-span-2 space-y-6">
          <SettingsSection title="Profile" description="Manage your personal information" icon={<User className="w-5 h-5" />}>
            <div className="flex items-center gap-4 mb-6">
              ...
              <div>
                <Button variant="ghost" size="sm" className="text-green hover:text-green-light">Change Avatar</Button>
              </div>
            </div>
          ...
          <SettingsSection title="Notifications" description="Choose how you want to be notified" icon={<Bell className="w-5 h-5" />}>
            <div className="space-y-4">
              <FormToggle label="Email Notifications" description="Receive updates via email" checked={notifications.email} onChange={(checked) => setNotifications({ ...notifications, email: checked })} />
              <div className="border-t border-white/5 pt-4">
                <FormToggle label="Execution Alerts" description="Get notified when workflows complete" checked={notifications.executions} onChange={(checked) => setNotifications({ ...notifications, executions: checked })} />
              </div>
              <div className="border-t border-white/5 pt-4">
                <FormToggle label="Error Alerts" description="Get notified when workflows fail" checked={notifications.errors} onChange={(checked) => setNotifications({ ...notifications, errors: checked })} />
              </div>
            </div>
          </SettingsSection>
          ...
          <SettingsSection title="Preferences" description="Customize your experience" icon={<Palette className="w-5 h-5" />}>
            <div className="space-y-4">
              <FormToggle label="Dark Mode" description="Use dark theme throughout the app" checked={preferences.darkMode} onChange={(checked) => setPreferences({ ...preferences, darkMode: checked })} />
              <div className="border-t border-white/5 pt-4">
                <FormToggle label="Auto-save" description="Automatically save workflow changes" checked={preferences.autoSave} onChange={(checked) => setPreferences({ ...preferences, autoSave: checked })} />
              </div>
            </div>
          </SettingsSection>
        </div>
